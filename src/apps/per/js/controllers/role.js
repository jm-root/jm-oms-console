...
app.controller('RoleCtrl', ['$scope', '$state', '$http', 'sso', function ($scope, $state, $http, sso) {
    ...
    $scope.getResource = function(roleCode){
        $http.get(aclUri+'/roles/_' + roleCode + '_/resources', {
            params:{
                token: sso.getToken()
                ,complex: true
            }
    ...
    $scope.updateResource = function(){
        ...
        if(resource){
            ...
            if(!isChecked){
                $http.delete(aclUri+'/roles/_'+role+'_/resources', {
                        params:{
                            token: sso.getToken()
                            ,resource: resource
                        }
    ...
    $scope.updatePermission = function(){
        ...
        if(permission){
            ...
            if(isChecked){
                $http.post(aclUri+'/roles/_'+role+'_/permissions', {}, {
                    params:{
                        token: sso.getToken()
                ...
                $http.delete(aclUri+'/roles/_'+role+'_/permissions', {
                        params:{
                            token: sso.getToken()
                            ,permissions: permission
                            ,resource: resource
    ...
    $scope.deleteRole = function(role){
        $scope.openTips({
            ...
            cancelTitle:'否',
            okCallback: function(){
                $http.delete(aclUri+'/roles/_'+role.code+'_', {
                    params:{
                        token: sso.getToken()
                    }
                }).success(function(result) {
